...
断线重连也做了，但**能做到的比"重连"这个词听起来的少**：厂商不给 SSE 事件分配稳定 `id`，
所以重连只能是「重发整个请求」，并且**一旦有内容吐给了 UI 就必须禁止重连**。理由见下文第 4 条。
...
| 指数退避 + 抖动的重连（语义是重发整个请求） | `ReconnectPolicy.swift` | `CloudReconnectTests`（7）+ `ReconnectPolicyTests`（4） |
...
- 🔴 **真正的流中途续传做不到**。已实现的是「失败即重发整个请求」，且只在还没吐出内容时允许。
  断点续传要么需要服务端给事件分配稳定 `id`（配合 `Last-Event-ID`），
  要么需要「幂等键 + 偏移量」协议 —— 两者都得后端配合，属于代理的设计问题。
  `SSEParser` 已经把 `id` / `retry` 解析出来了，只是**没人给**，所以上层没有消费它们。
...
- ⬜ **429 不自动重试**。`.rateLimited(retryAfter:)` 把服务端给的秒数如实带出来，
  但重连策略刻意把它排除在可重试之外 —— 服务端已经明确说了「慢点」，
  立刻重试是变本加厉。等多久由调用方决定。
...
### 4. 「重连」实际上只能是「重发整个请求」，因此吐过内容之后就不能再重连

SSE 规范给了续传机制：服务端用 `id:` 给事件编号，客户端断线后带 `Last-Event-ID`
重连，服务端从那之后接着发。**但大模型厂商的流普遍不给事件分配 `id`** ——
`SSEParser` 早就把 `id` 解析出来了，实测中就是没人给。没有 id 就无从告诉服务端
「我收到哪儿了」，断线后只能从头再来一次。

「从头再来」会拿回一段**全新的、与已吐内容无关的**文本（采样是随机的，
即使 prompt 一样也不会逐字重现）。如果此前已经有内容吐给了 UI，重发只有两个结局：

- 让 UI 上的文字凭空跳变 —— 违反「每片必须是前一片的延展」这条不变量；
- 或者假装两段能接上 —— 那是**伪造连续性**，比如实报错更糟。

所以 `ReconnectPolicy.retrying` 收一个 `canRetry` 闭包，流式路径传的是 `{ !delivered }`：
只要有一个字符出过门，重连就被永久禁掉，断连如实抛 `.network`。
`doesNotReconnectAfterDeliveringContent` 这条测试把它钉死了 —— 它故意让第二次请求会成功，
这样「偷偷重发」一旦发生就会被抓住（断言请求数恰好是 1）。

另外两个刻意的取舍：

- **只有 `.network` 可重试。** 其余每一条都写了排除理由（见 `ReconnectPolicy` 的注释）：
  凭证问题重试无用、429 是服务端叫你慢点、请求本身有问题重发必然同样失败、
  超时预算已经花掉了。
- **重试次数默认只有 2 次，且带抖动。** 重试次数是**乘在服务端负载上**的：
  服务端正在雪崩时，客户端多重试一次就是给它多加一倍压力；
  不加抖动则会让所有客户端在同一时刻一起重试，把一次抖动放大成一波雪崩。
- **退避不自己管总时长**，全部发生在调用方的超时预算之内，由 `ModelRouter.withTimeout`
  统一切断。两处都管总时长会互相打架，而且会让「到底谁把我掐了」难以排查。
  `backoffStaysInsideTimeoutBudget` 用 10 秒退避 + 400ms 超时验证这条。
...
起了一个真的 HTTP/1.1 服务器，走真 socket、真 `URLSession`。五种应答模式：
...
| `.truncated(declaredLength:pieces:...)` | 声明了 `Content-Length` 却只发一部分就关连接，测流中途断线 |
| `.hang` | 接受连接后什么都不回也不关，测超时兜底 |

另外服务器接受 `replies: [Reply]` 数组，**第 N 个连接用第 N 条应答**，用完最后一条就一直重复它。
这是测重试路径的前提 —— 「先失败两次、第三次成功」必须能表达出来。
...
- 不带 `Content-Length` 的响应用 `Connection: close` 界定结束 ——
  ...
  **反过来说，这就是为什么模拟「流中途断线」必须声明 `Content-Length`**：
  close-delimited 的响应提前关连接等于正常收尾，`URLSession` 根本不会报错；
  声明了长度却不给够，才是一个明确的截断。已实测确认 `URLSession` 会为此抛错。

## 实测数据

| 指标 | 环境 | 数值 | 备注 |
| --- | --- | --- | --- |
| 逻辑测试耗时 | 宿主 macOS 15.7.3，`swift test` | AICore 66 tests / 9 suites ≈ 0.43s；AIFeatures 7 tests ≈ 0.46s | 其中 26 个用例真的起了 socket |
| 超时兜底生效 | `.hang` 服务器，请求超时设 400ms | 流式与非流式均在 5s 内抛 `.timedOut` | 契约是「一定会返回」，不是「哪一层先超时」 |
| 退避不逃逸超时预算 | 退避设 10s，请求超时设 400ms | 在 5s 内抛 `.timedOut` | 重试不能变成绕过超时的后门 |
...
- 默认装配**仍然是 mock**（`ProviderFactory.makeDefaultRouter()`），
  因为没有后端代理，`baseURL` 无处可指。真实装配走
  `ProviderFactory.makeRouter(cloudBaseURL:model:credential:reconnect:)`，
  `reconnect` 默认开（重试 2 次），要关传 `.disabled`。
- 待验证问题（已登记到 [`../backlog.md`](../backlog.md)）：
  - 流中途续传要不要做（需要后端提供稳定事件 `id`，或「幂等键 + 偏移量」协议）
